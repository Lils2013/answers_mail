...
def ram_save_question(qdata):
    ...
    category = question_m.get(qdata['cat_id'])
    if category is None:
...
def ram_to_db():
    ...
    existing_categories = Category.objects.filter(id__in=category_m.keys())
    ...
    existing_questions = Question.objects.filter(id__in=question_m.keys())
    for iid, data in question_m.iteritems():
        if iid in existing_questions:
            continue
        else:
            question = Question(text=data['text'], created_at=data['created_at'], id=iid, rating=data['rating'], category_id=data["category"])
            question.save()
    ...
    for category_id, d1 in counter_m.iteritems():
        for tag_text, d2 in d1.iteritems():
            for date_time, cnt in d2.iteritems():
                ...
                counter = Counter.objects.all().filter(category_id=category_id, tag_id=tag_m[tag_text]["id"], datetime=date_time)
                if not counter:
                    counter = Counter(category_id=category_id, tag_id=tag_m[tag_text]["id"], datetime=date_time, count=cnt["count"])
                    counter.save()
                else:
                    counter = counter[0]
